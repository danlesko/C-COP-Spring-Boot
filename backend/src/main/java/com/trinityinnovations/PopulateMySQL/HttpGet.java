 ...
 */
  ...
  public static void main(String[] args) {
    ...
    class ArrestEntry {
      ...
      public void setStreet(String street) throws InterruptedException, ApiException, IOException {
        ...
        if (street.length() > 0 && street.equals("NO FIXED ADDRESS")) {}
          ...
          try {
            results = GeocodingApi.geocode(context, street).await();
          ...
          if (results.length > 0) {
            ...
            setLatitude(Double.valueOf(results[0].geometry.location.lat));
            setLongitude(Double.valueOf(results[0].geometry.location.lng));
          }
        }
      }
    ...
    try {
      ...
      // SQL to talk to database
      String sql = "insert into arrest (last_name, first_name, middle_name, age, street, city, state, arrest_date, offense, latitude, longitude)" +
        "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
